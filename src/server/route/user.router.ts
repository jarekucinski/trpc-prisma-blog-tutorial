...
export const userRouter = createRouter()
  .mutation('register-user', {
    ...
    async resolve({ ctx, input }) {
      ...
      try {
        ...
        throw new trpc.TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Something went wrong' })
      }
    },
  })
  ...
  .query('verify-otp', {
    ...
    async resolve({ input, ctx }) {
      ...
      if (!token) {
        throw new trpc.TRPCError({ code: 'FORBIDDEN', message: 'Invalid token' })
      }
      ...
      ctx.res.setHeader('Set-Cookie', serializeCookie('token', jwt, { path: '/' }))
